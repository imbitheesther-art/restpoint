...
class ConnectionManager {
     ...
     */
    async execute(tenantDbName, sql, params = []) {
        if (this.isShuttingDown) {
     ...
     */
    async getOrCreatePool(tenantDbName) {
        ...
        pool.on('release', () => {
            ...
            const meta = this.poolCache.get(tenantDbName);
            if (meta && meta.activeConnections > 0)
                meta.activeConnections--;
        });
        pool.on('connection', () => {
        ...
        try {
            ...
            throw new Error(`[ConnectionManager] ❌ Failed to establish pool for "${tenantDbName}": ${initError.message}`);
        }
     ...
     */
    async acquireConnection(metadata, tenantDbName) {
        const timeoutPromise = new Promise((_, reject) => {
            setTimeout(() => {
                reject(new Error(`Timed out waiting for connection from pool "${tenantDbName}" after ${this.config.acquireTimeoutMs}ms`));
     ...
     */
    async validateConnection(connection, tenantDbName) {
        try {
            ...
            console.warn(`[ConnectionManager] ⚠️ Dead connection detected for "${tenantDbName}": ${validationError.message}. Pruning...`);
    ...
    async evictPool(tenantDbName) {
        ...
        try {
            ...
            console.warn(`[ConnectionManager] ⚠️ Pool drain for "${tenantDbName}" may have incomplete connections: ${drainError.message}`);
        }
...
const safeExecute = async (tenantDbName, sql, params = []) => {
    return exports.connectionManager.execute(tenantDbName, sql, params);
};
