...
class MigrationService {
     ...
     */
    async initializeMasterConnection(config) {
        this.masterPool = promise_1.default.createPool({
            host: config.host,
            port: config.port,
            user: config.user,
     ...
     */
    async runTenantMigrations(dbName, migrations, connectionConfig, onProgress) {
        const result = {
            success: true,
            migrationsRun: [],
            errors: [],
        ...
        try {
            ...
            for (const migration of migrations) {
                ...
                try {
                    ...
                    await connection.query('INSERT INTO migrations (migration_name, executed_at) VALUES (?, NOW())', [migration.name]);
     ...
     */
    async rollbackMigrations(dbName, migrations, count, connectionConfig) {
        const result = {
            success: true,
            migrationsRun: [],
            errors: [],
        ...
        try {
            ...
            for (const migrationName of executedList) {
                ...
                try {
                    ...
                    await connection.query('DELETE FROM migrations WHERE migration_name = ?', [migration.name]);
                    result.migrationsRun.push(migration.name);
