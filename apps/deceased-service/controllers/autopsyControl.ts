...
const generateReportPdfBuffer = async (data) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Right column
      addMetadataRow('Date:', 
        new Date(date || created_at).toLocaleDateString('en-GB', {
          day: '2-digit',
          month: 'short',
          year: 'numeric'
        }), 
        col2X, currentY);
      addMetadataRow('Mortuary:', 
        mortuary_name || 'Lee Funeral Services', 
        col2X, currentY + lineHeight);
      addMetadataRow('Pathologist:', 
        pathologist_name || staff_username || 'Dr. Peter Mumo', 
        col2X, currentY + (lineHeight * 2));
        ...
        findingsKeys.forEach((key) => {
          ...
          doc.font('Helvetica-Bold').fontSize(7).fillColor('#1a5276')
            .text(key.charAt(0).toUpperCase() + key.slice(1), 
                  margin + 5, currentY + 6, 
                  { width: organColWidth - 10, align: 'left' });
          ...
          doc.font('Helvetica').fontSize(7).fillColor('#333333')
            .text(checkedFinding, 
                  margin + organColWidth + 5, currentY + 6, 
                  { width: findingColWidth - 10, lineGap: 1, align: 'left' });
...
//Final  Determination
      ...
      if (fs.existsSync(forensicSignaturePath)) {
        ...
        doc.fontSize(5.5).font('Helvetica-Bold').fillColor('#333333')
          ...
          .text(pathologist_name || 'Dr. Lee  FH  Forensic Department', 
                signatureX, currentY + 28, { width: 80, align: 'center' });
      } else {
      ...
      doc.fontSize(5).font('Helvetica').fillColor('#1a5276')
        .text('This report is generated for medical and legal purposes. The findings are based on the examination', 
              margin, currentY, { width: contentWidth, align: 'left' });
      
      currentY += 4;
      
      doc.fontSize(5).font('Helvetica').fillColor('#1a5276')
        .text('conducted at the time of autopsy. Spell checking has been applied automatically but medical', 
              margin, currentY, { width: contentWidth, align: 'left' });
      
      currentY += 4;
      
      doc.fontSize(5).font('Helvetica').fillColor('#1a5276')
        .text('terminology accuracy should be verified by qualified personnel. This document is confidential.', 
              margin, currentY, { width: contentWidth, align: 'left' });
      ...
      Logger.info('PDF document finalized, streaming started', {  //start   streaming   data 
      ...
      writeStream.on("finish", () => {
        ...
        Logger.info('PDF write completed, starting gzip compression', {   // start  gzip  compression
      ...
      output.on("error", reject);
       ...
       input.pipe(gzip).pipe(output);
        ...
        output.on("finish", () => {
          ...
          Logger.info('PDF gzip compression completed successfully', {  //gzip  compressin sucess
...
const checkColumnExists = async (columnName) => {
  try {
    const tableInfo = await safeQuery(
      `SHOW COLUMNS FROM postmortem LIKE ?`,
      [columnName]
    );
...
const checkAutopsyIdExists = async (autopsyId) => {
  try {
    const existingRows = await safeQuery(
      `SELECT autopsy_id FROM postmortem WHERE autopsy_id = ?`,
      [autopsyId]
    );
...
const registerAutopsy = asyncHandler(async (req, res) => {
  ...
  if (staff_username) {
    ...
    const systemRows = await safeQuery(
      `SELECT id, username 
       FROM users 
