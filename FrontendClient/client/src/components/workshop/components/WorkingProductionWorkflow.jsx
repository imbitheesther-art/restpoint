...
const WorkingProductionWorkflow = () => {
    ...
    const loadData = async () => {
        ...
        try {
            // Load coffin orders
            const ordersRes = await fetch(`/api/coffin-orders?tenant=${tenant.db_name}`);
    ...
    const createOrder = async () => {
        try {
            const response = await fetch('/api/coffin-orders', {
                method: 'POST',
                headers: {
    ...
    const updateOrderStage = async (orderId, stage) => {
        try {
            const response = await fetch(`/api/coffin-orders/${orderId}/stage`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'x-tenant-db': tenant.db_name
    ...
    const assignWorker = async (orderId, workerId) => {
        try {
            const response = await fetch(`/api/coffin-orders/${orderId}/assign`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'x-tenant-db': tenant.db_name
    ...
    return (
        <div style={{ padding: '20px', background: '#f8f9fa', minHeight: '100vh' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                <h1>🏭 Production Workflow System</h1>
                <button
...
export default WorkingProductionWorkflow;