...
export const ENDPOINTS = {
  ...
  LEAVE: {
    BASE: '/leaves',
    APPLY: (slug) => `/leaves/${slug}/apply`,
    ALL: (slug) => `/leaves/${slug}/all`,
    MY_LEAVES: (slug) => `/leaves/${slug}/my-leaves`,
    DETAIL: (slug, id) => `/leaves/${slug}/${id}`,
    UPDATE_STATUS: (slug, id) => `/leaves/${slug}/${id}/status`,
    CANCEL: (slug, id) => `/leaves/${slug}/${id}/cancel`,
    UPLOAD_DOCUMENT: (slug, id) => `/leaves/${slug}/${id}/upload-document`,
    STATS: (slug) => `/leaves/${slug}/stats/overview`,
    USERS_ON_LEAVE: (slug) => `/leaves/${slug}/users/on-leave`,
  },
};
