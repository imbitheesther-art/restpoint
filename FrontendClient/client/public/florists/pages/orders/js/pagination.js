...
const initializePagination = () => {
    ...
    if (pageData && pageData[0] && pageData[0].searchParam) {
        fetchResults(currentPage, pageData[0].searchParam);
        const searchInputElement = document.getElementById('search-input');
        if (searchInputElement) searchInputElement.value = pageData[0].searchParam;
        searchInput = pageData[0].searchParam;
    } else {
        fetchResults(currentPage);
    }
};
...
if (window.ipcRenderer && typeof window.ipcRenderer.on === 'function') {
    window.ipcRenderer.on('page-data', (event, data) => {
        if (data) {
            if (data[0]) {
