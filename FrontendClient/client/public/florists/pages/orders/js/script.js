...
function addOrderRow(data) {
    ...
    // Description will only show the first line, and no more than 100 characters
    const notes = data.order_notes || data.description || '';
    ...
    orderIdCell.textContent = data.order_id || data.order_number || '—';

    var jobNumberCell = document.createElement('td');
    jobNumberCell.className = 'order-column';
    jobNumberCell.textContent = data.branch || data.delivery_branch || data.delivery_time || data.job_number || '—';
    ...
    dateDueCell.textContent = data.date_due || data.delivery_date || '—';

    var invoiceNumberCell = document.createElement('td');
    invoiceNumberCell.className = 'order-column invoice-number-column';
    invoiceNumberCell.textContent = data.order_status || data.status || data.invoice_number || 'Pending';
...
searchbtn.addEventListener('click', (e) => {
    ...
    if (filterValue === 'date-range') {
        ...
        params.append('status', searchInput)
...
const pageNavigationMap = {
    'dashboard-page-btn': 'orders',
    'today-orders-page-btn': 'orders',
    'new-order-page-btn': 'order-detail',
    'preparing-page-btn': 'orders',
    'ready-delivery-page-btn': 'orders',
    'out-delivery-page-btn': 'orders',
    'completed-page-btn': 'orders',
    'cancelled-page-btn': 'orders',
    'customers-page-btn': 'customers',
    'branches-page-btn': 'invoices',
    'calendar-page-btn': 'orders',
    'reports-page-btn': 'invoices',
    'settings-page-btn': 'customers',
    'profile-page-btn': 'customers',
    'orders-page-btn': 'orders',
    'invoices-page-btn': 'invoices'
};

Object.entries(pageNavigationMap).forEach(([navId, page]) => {
    const el = document.getElementById(navId);
    if (!el) return;
    el.addEventListener('click', () => {
        if (page === 'order-detail') {
            navigateTo(page, { showEditAndDelete: false });
        } else {
            navigateTo(page);
        }
    });
